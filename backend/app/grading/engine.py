...
def grade_criterion_exemplar(
    ...
    essay_text: str, both_paths_ctx: BothPathsContext,
) -> PassResult:
    ...
    return _run_graded_pass(client, system_prompt, essay_text, pool)


def grade_criterion_personalized(
    conn: sqlite3.Connection, client: LLMClient, criterion: dict, rubric_id: str,
    essay_text: str, both_paths_ctx: BothPathsContext, scope: Scope,
) -> PassResult:
...
def run_dual_path_for_criterion(
    ...
    instructor_id: str, course_id: str | None,
) -> None:
    ...
    result_e = grade_criterion_exemplar(client, criterion, rubric_id, rubric_version, essay_text, both_paths_ctx)
    _persist_score_record(conn, assessment_id, criterion["criterionId"], "exemplar", result_e)

    result_p = grade_criterion_personalized(conn, client, criterion, rubric_id, essay_text, both_paths_ctx, scope)
